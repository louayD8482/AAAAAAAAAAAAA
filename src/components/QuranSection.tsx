...
export default function QuranSection({ isEn = false }: QuranSectionProps) {
  ...
  const [fontSize, setFontSize] = useState<number>(24); // default slightly larger for Arabic clarity
  ...
  // Handler to update Khatma Progress
  ...
  const [lookupSurah, setLookupSurah] = useState<number>(18); // default Al-Kahf
  ...
  const handleSelectSurah = (surahMeta: SurahMetadata) => {
    // Clear previous Tafsir states
    setTafsirResult(null);
    setActiveTafsirAyah(null);

    // If we have full offline text, load it
    if (offlineSurahs[surahMeta.number]) {
      setSelectedSurah(offlineSurahs[surahMeta.number]);
    } else {
      // Create empty/metadata-only Surah detail to allow requesting dynamic Tafsir
      setSelectedSurah({
        ...surahMeta,
        ayahs: [],
      });
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pb-2 border-b border-slate-100 dark:border-slate-800/40">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-lg font-extrabold text-slate-800 dark:text-slate-100">المصحف الشريف وتفسير السور</h3>
            </div>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              تصفح سور القرآن الـ 114 واطلب تفسيراً لأي سورة أو آية بالذكاء الاصطناعي
            </p>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2 border-t border-slate-200/30 dark:border-slate-800/30">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {/* Manual numeric input with adjuster */}
            ...
            <form onSubmit={handleManualAyahLookup} className="flex flex-wrap gap-2.5 items-end">
              <div className="flex flex-col gap-1 text-xs">
                ...
                <select
                  ...
                  className="px-2.5 py-1.5 bg-white dark:bg-slate-950 border border-emerald-100 dark:border-slate-800 rounded-lg text-xs font-bold"
                >
                  {quranMetadata.map(s => (
              ...
              <button
                ...
                className="px-4 py-1.5 bg-emerald-700 hover:bg-emerald-800 text-white text-xs font-bold rounded-lg transition-colors shadow-xs flex items-center gap-1.5 h-8"
              >
          ...
          <div className="flex flex-col gap-2.5">
            {filteredSurahs.map((surah) => {
              const isOfflineAvailable = !!offlineSurahs[surah.number];
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-slate-100 dark:border-slate-800/40">
            ...
            >
              ...
              <span>العودة للمصحف الشريف</span>
            </button>
          ...
          <div className="text-center p-6 bg-gradient-to-b from-emerald-800 to-teal-950 text-white rounded-3xl space-y-2 relative overflow-hidden">
            ...
            <h3 className="text-3xl font-extrabold text-amber-300 mt-2">{selectedSurah.name}</h3>
            <p className="text-xs text-emerald-100/80">{selectedSurah.numberOfAyahs} آيات كريمة</p>
            ...
            <button
              ...
              className="mt-4 inline-flex items-center gap-1.5 px-4 py-2 bg-amber-400 hover:bg-amber-500 text-slate-950 font-bold text-xs rounded-full shadow-md hover:shadow-lg transition-all"
            >
          ...
          <div className="p-4 bg-slate-50/80 dark:bg-slate-950/40 border border-slate-100 dark:border-slate-800/80 rounded-2xl space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200/50 dark:border-slate-800/50 pb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-slate-800 dark:text-slate-200">الاستماع للتلاوة العطرة بصوت كبار القراء</span>
              </div>
          ...
          <div className="space-y-6">
            {selectedSurah.ayahs && selectedSurah.ayahs.length > 0 ? (
              /* Display offline full verses if available */
              <div className="p-6 bg-slate-50/50 dark:bg-slate-950/40 border border-slate-100/40 dark:border-slate-850 rounded-2xl space-y-6 text-center">
                ...
                {/* Bismillah (skip for Al-Tawbah (9) and Al-Fatihah (1) because it is already first verse) */}
                ...
                {selectedSurah.ayahs.map((ayah) => (
                  ...
                  >
                    ...
                    {/* Offline Quick Tafsir text toggle */}
              ...
              /* Display details card for online-only Surahs */
              <div className="p-6 bg-slate-50/50 dark:bg-slate-950/40 border border-slate-100/40 dark:border-slate-850 rounded-2xl text-center space-y-4">
                <AlertCircle className="w-12 h-12 text-emerald-700 mx-auto" />
                <h4 className="text-lg font-bold text-slate-800 dark:text-slate-100">سورة {selectedSurah.name} جاهزة للقراءة والتفسير</h4>
                <p className="text-xs text-slate-500 dark:text-slate-400 max-w-md mx-auto leading-relaxed">
                  للحفاظ على حجم التطبيق وسرعته، نتيح قراءة وفهم وتفسير السور غير المحملة محلياً عبر محرك الذكاء الاصطناعي لؤي بن حسين. انقر على الزر بالأسفل لجلب تفسير وتحليل كامل مقاصد وآيات سورة {selectedSurah.name}.
                </p>
                
                <button
                  id="surah-online-tafsir-btn"
                  onClick={() => handleFetchSurahTafsir(selectedSurah.number, selectedSurah.name)}
                  className="px-5 py-2.5 bg-emerald-700 hover:bg-emerald-800 text-white text-xs font-bold rounded-xl transition-all shadow-md flex items-center gap-1.5 mx-auto"
                >
                  <Sparkles className="w-4 h-4 text-amber-300" />
                  جلب مقاصد وتفسير سورة {selectedSurah.name} الآن
                </button>
              </div>
            )}
          </div>
